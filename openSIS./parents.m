   ...
   NSString*str_checklogin=[NSString stringWithFormat:@"/student_info.php?staff_id=%@&school_id=%@&syear=%@&student_id=%@",STAFF_ID_K,school_id1,year_id,self.studentID];
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if([str_123 isEqualToString:@"1"])
        {
            ...
            array_cert = [dictionary1 objectForKey:@"parent_data"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  //  NSString *end_date = [self nullChecker:[NSString stringWithFormat:@"%@",[[array_cert objectAtIndex:indexPath.row] objectForKey:@"END_DATE"]]];
    ...
    cell.lbl_Certname.text  = [self nullChecker:[NSString stringWithFormat:@"%@",[[array_cert objectAtIndex:indexPath.row] objectForKey:@"FULL_NAME"]]];
...
//   NSDateFormatter *dateFormatter1 = [[NSDateFormatter alloc] init] ;
  ...
  //  NSLog(@"Converted String : %@",convertedString);
    cell.lbl_Certcode.text=[self nullChecker:[NSString stringWithFormat:@"%@",[[array_cert objectAtIndex:indexPath.row] objectForKey:@"HOME_PHONE"]]];
    cell.lbl_certDesc.text=[self nullChecker:[NSString stringWithFormat:@"%@",[[array_cert objectAtIndex:indexPath.row] objectForKey:@"EMAIL"]]];
    cell.lbl_validfrom.text=[self nullChecker:[NSString stringWithFormat:@"%@",[[array_cert objectAtIndex:indexPath.row] objectForKey:@"RELATIONSHIP"]]];
