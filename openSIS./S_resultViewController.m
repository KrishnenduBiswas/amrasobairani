...
#import "S_resultViewController.h"
...
@interface S_resultViewController ()
{
...
@implementation S_resultViewController
{
...
-(void)loaddata1
{
    ...
    NSString*str_checklogin=[NSString stringWithFormat:@"/student/stu_message_group.php?student_id=%@&school_id=%@&syear=%@&action_type=add_grp_member_view&grp_id=%@&last=%@&first=%@&username=%@&profile=%@&_dis_user=%@&_search_all_schools=%@",student_id,s_year_id,s_year,self.group_id,self.str_last,self.str_first,self.str_usr,self.str_profile,self.str_disusr,self.str_allsch];
