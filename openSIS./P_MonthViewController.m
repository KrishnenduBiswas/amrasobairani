...
@interface P_MonthViewController ()<CLWeeklyCalendarViewDelegate,UIPickerViewDelegate,UIPickerViewDataSource,VRGCalendarViewDelegate>
...
@implementation P_MonthViewController
...
//    NSString *STAFF_ID_K=[DF objectForKey:@"iphone"];
    ...
    NSString*str_checklogin=[NSString stringWithFormat:@"/parent/parent_calendar.php?parent_id=%@&student_id=%@&syear=%@&action_type=%@&view_type=month&date=%@&calendar_id=",parentid,student_id,s_year,assignment_data,date_value];    NSLog(@"kkkkkkkkkkk%@",str_checklogin);
...
-(void)loaddata1
{
    ...
    ip_url *obj123=[[ip_url alloc]init];
    NSString *str123=[obj123 ipurl];
    NSString*str_checklogin=[NSString stringWithFormat:@"/parent/parent_calendar.php?parent_id=%@&student_id=%@&syear=%@&action_type=%@&view_type=month&date=%@&calendar_id=",parentid,student_id,s_year,assignment_data,date_value];
...
- (WeekViewCell1 *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    if (flag1==1) {
        ...
        cell.lbl_time.text = [self nullChecker:[NSString stringWithFormat:@"%@",[[ary objectAtIndex:indexPath.row]objectForKey:@"TITLE"]]];
    }
    ...
    else
    {
        ...
        cell.day1.text=[self nullChecker:[NSString stringWithFormat:@"%@",[[ary objectAtIndex:indexPath.row]objectForKey:@"ASSIGNED_DATE"]]];
        
        
        cell.lbl_time.text = [self nullChecker:[NSString stringWithFormat:@"%@",[[ary objectAtIndex:indexPath.row]objectForKey:@"TITLE"]]];
        cell.lbl_period.text = [self nullChecker:[NSString stringWithFormat:@"%@",[[ary objectAtIndex:indexPath.row]objectForKey:@"DESCRIPTION"]]];
        cell.lbl_term.text = [self nullChecker:[NSString stringWithFormat:@"%@",[[ary objectAtIndex:indexPath.row]objectForKey:@"DUE_DATE"]]];
...
- (void)dismisspicker
{
   ...
   // UIStoryboard *sb = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
    ...
    if ([self.text_mon_day_yr.text isEqualToString:@"List View"]) {
        UIStoryboard *s=[UIStoryboard storyboardWithName:@"ParentStoryboard" bundle:nil];
        P_ListViewController *obj=[s instantiateViewControllerWithIdentifier:@"p_list"];
        [self.navigationController pushViewController:obj animated:YES];
