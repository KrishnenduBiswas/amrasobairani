...
@interface S_CLWeeklyCalendarView()<DailyCalendarViewDelegate, UIGestureRecognizerDelegate>
...
@implementation S_CLWeeklyCalendarView
...
-(S_DailyCalendarView *)dailyViewForDate: (NSDate *)date inFrame: (CGRect)frame
{
    S_DailyCalendarView *view = [[S_DailyCalendarView alloc] initWithFrame:frame];
    view.date = date;
...
{
    for (S_DailyCalendarView *v in [self.dailySubViewContainer subviews]){
...
-(void)renderSwipeDates: (NSDictionary*)param
{
    ...
    for(int i = 0; i < WEEKLY_VIEW_COUNT; i++){
        ...
        S_DailyCalendarView* view = [self dailyViewForDate:dt inFrame: CGRectMake(dailyWidth*i, 0, dailyWidth, DATE_VIEW_HEIGHT) ];
