...
@interface Month1ViewController ()<CLWeeklyCalendarViewDelegate,UIPickerViewDelegate,UIPickerViewDataSource,VRGCalendarViewDelegate>
...
@implementation Month1ViewController
...
- (void)viewDidLoad {
    ...
    date_value=[df stringFromDate:[NSDate date]];
    ...
    NSString*str_checklogin=[NSString stringWithFormat:@"/calender.php?staff_id=%@&school_id=%@&syear=%@&action_type=%@&view_type=month&date=%@&calendar_id=%@",STAFF_ID_K,school_id123456,str_school_year,assignment_data,date_value,school_id123456];
    NSLog(@"kkkkkkkkkkk%@",str_checklogin);
   ...
   NSString *str_current_date;//=[NSString stringWithFormat:@"%@",[NSDate date]];
    ...
    [df11 setDateFormat:@"MMM dd, yyyy"];
    
    
    str_current_date=[df11 stringFromDate:[NSDate date]];
...
- (void)dismisspicker
{
    ...
    if ([self.text_mon_day_yr.text isEqualToString:@"Week"]) {
     ...
     //   [self.navigationController popViewControllerAnimated:YES];
        ...
        UIStoryboard *sb=[UIStoryboard storyboardWithName:@"schoolinfo" bundle:nil];
        Month1wViewController *obj = [sb instantiateViewControllerWithIdentifier:@"week1"];
        [self.navigationController pushViewController:obj animated:YES];
