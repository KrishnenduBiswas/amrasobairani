...
@implementation CourseManagerPeriod
{
...
- (void)viewDidLoad {
    ...
    [NSTimer scheduledTimerWithTimeInterval:2.0
                                   ...
                                   userInfo:nil
                                    repeats:YES];
    _lbl_blueheader.text=self.str_header_lbl;
...
-(void)loaddata1
{
   ...
   //  http://107.170.94.176/openSIS_CE6_Mobile/webservice/course_manager.php?staff_id=2&school_id=1&syear=2015&mp_id=17&view_type=period&subject_id=2&course_id=18&cp_id=
    ...
    NSString*str_checklogin=[NSString stringWithFormat:@"/course_manager.php?staff_id=%@&school_id=%@&syear=%@&mp_id=%@&view_type=period&subject_id=%@&course_id=%@&cp_id=",STAFF_ID_K,school_id1,year_id,mp_id,self.subject_id, self.course_id];
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        //    NSUserDefaults *obj12=[NSUserDefaults standardUserDefaults];
        NSString *slectedSub = [NSString stringWithFormat:@"%@",[dictionary1 objectForKey:@"selected_course"]];
        
        array_forpicker = [[NSMutableArray alloc]init];
        array_forpicker = [dictionary1 objectForKey:@"courses"];
        
        for (int i =0; i < array_forpicker.count; ++i) {
            NSString *temp = [[array_forpicker objectAtIndex:0] objectForKey:@"COURSE_ID"];
...
{
    CourseManagerPeriodCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CourseManagerPeriodCell"];
    ...
    cell.lbl_count.text = [self nullChecker:[NSString stringWithFormat:@"%@", [[array_subjects objectAtIndex:indexPath.row] objectForKey:@"AVAILABLE_SEATS"]]];
    cell.lbl_credit.text = [self nullChecker:[NSString stringWithFormat:@"%@", [[array_subjects objectAtIndex:indexPath.row] objectForKey:@"CREDITS"]]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CourseManagerPeriodDetails *obj  = [sb instantiateViewControllerWithIdentifier:@"CourseManagerPeriodDetails"];
...

{
    if (pickerView == selectcustomerpicker) {
        ...
        self.course_id = [[array_forpicker objectAtIndex:row] objectForKey:@"COURSE_ID"];
