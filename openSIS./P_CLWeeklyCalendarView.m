...
@interface P_CLWeeklyCalendarView()<DailyCalendarViewDelegate, UIGestureRecognizerDelegate>
...
@implementation P_CLWeeklyCalendarView
...
-(UIView *)dailyInfoSubViewContainer
{
    if(!_dailyInfoSubViewContainer){
        _dailyInfoSubViewContainer = [[UIView alloc] initWithFrame:CGRectMake(0, DATE_TITLE_MARGIN_TOP+DAY_TITLE_VIEW_HEIGHT + DATE_VIEW_HEIGHT + DATE_VIEW_MARGIN * 2, self.bounds.size.width, DATE_LABEL_INFO_HEIGHT+25)];
...
-(UIView *)dayTitleSubViewContainer
{
    if(!_dayTitleSubViewContainer){
        ...
        _dayTitleSubViewContainer.backgroundColor = [UIColor colorWithHex:0x99CA5A];
        _dayTitleSubViewContainer.layer.borderColor = [[UIColor colorWithHex:0x7AA248] CGColor];
        _dayTitleSubViewContainer.layer.borderWidth = 1.0f;
...
-(UIView *)dailySubViewContainer
{
    if(!_dailySubViewContainer){
        ...
        //        _dailySubViewContainer = [[UIImageView alloc] initWithFrame:CGRectMake(0, DATE_TITLE_MARGIN_TOP+DAY_TITLE_VIEW_HEIGHT, self.bounds.size.width, DATE_VIEW_HEIGHT)];
        _dailySubViewContainer = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0 + DAY_TITLE_VIEW_HEIGHT, self.bounds.size.width, DATE_VIEW_HEIGHT+25)];
...
-(P_DailyCalendarView *)dailyViewForDate: (NSDate *)date inFrame: (CGRect)frame
{
    P_DailyCalendarView *view = [[P_DailyCalendarView alloc] initWithFrame:frame];
    view.date = date;
...
{
    for (P_DailyCalendarView *v in [self.dailySubViewContainer subviews]){
...
-(void)adjustDailyInfoLabelAndWeatherIcon: (BOOL)blnShowWeatherIcon
{
    ...
    if(blnShowWeatherIcon){
        ...
        self.dateInfoLabel.frame = CGRectMake( (self.bounds.size.width - DATE_LABEL_INFO_WIDTH)/2, 10, DATE_LABEL_INFO_WIDTH, DATE_LABEL_INFO_HEIGHT);
    }
...
-(void)renderSwipeDates: (NSDictionary*)param
{
    ...
    for(int i = 0; i < WEEKLY_VIEW_COUNT; i++){
        ...
        P_DailyCalendarView* view = [self dailyViewForDate:dt inFrame: CGRectMake(dailyWidth*i, 0, dailyWidth, DATE_VIEW_HEIGHT) ];
