...
@interface S_ListViewController()<CLWeeklyCalendarViewDelegate,UIPickerViewDelegate,UIPickerViewDataSource,VRGCalendarViewDelegate>
...
@implementation S_ListViewController
...
//    NSString *STAFF_ID_K=[DF objectForKey:@"iphone"];
    ...
    NSString*str_checklogin=[NSString stringWithFormat:@"/student/stu_calendar.php?student_id=%@&school_id=%@&syear=%@&action_type=%@&view_type=list&date=%@&calendar_id=",student_id,s_year_id,s_year,assignment_data,date_value];
...
//    NSString *STAFF_ID_K=[DF objectForKey:@"iphone"];
    ...
    ip_url *obj123=[[ip_url alloc]init];
    NSString *str123=[obj123 ipurl];
    NSString*str_checklogin=[NSString stringWithFormat:@"/student/stu_calendar.php?student_id=%@&school_id=%@&syear=%@&action_type=%@&view_type=list&date=%@&calendar_id=",student_id,s_year_id,s_year,assignment_data,date_value];    NSLog(@"kkkkkkkkkkk%@",str_checklogin);
    NSString *url12=[NSString stringWithFormat:@"%@%@",str123,str_checklogin];
...
- (list1 *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    cell.lbl_time.text = [NSString stringWithFormat:@"%@",[[ary objectAtIndex:indexPath.row]objectForKey:@"SCHOOL_DATE"]];
    cell.lbl_period.text = [NSString stringWithFormat:@"%@",[[ary objectAtIndex:indexPath.row]objectForKey:@"DESCRIPTION"]];
...
- (void)dismisspicker
{
    ...
    if ([self.text_mon_day_yr.text isEqualToString:@"Month"]) {
        UIStoryboard *s=[UIStoryboard storyboardWithName:@"S_Calender" bundle:nil];
        S_MonthViewController *obj=[s instantiateViewControllerWithIdentifier:@"s_month"];
        [self.navigationController pushViewController:obj animated:YES];
