...
-(void)onloaddata
{
    ...
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [self performSelector:@selector(onloaddata1) withObject:nil afterDelay:1.0];
        });
    });
...
-(void)onloaddata1
{
    ...
    NSUserDefaults *df=[NSUserDefaults standardUserDefaults];
    NSString *std_id=[df objectForKey:@"std_id"];
    NSString*str_checklogin=[NSString stringWithFormat:@"/goal_details.php?staff_id=%@&school_id=%@&syear=%@&student_id=%@&&goal_id=%@",STAFF_ID_K,school_id1,year_id,self.studentID,self.goal_id];
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        NSLog(@"value  tanay is-------%@",dictionary1); // ei khanei nslog korle dekhache, blocker baire dekhache na
       ...
       NSMutableArray *ary_data=[[NSMutableArray alloc]init];
        ...
        //    NSLog(@"tanay---%@",ary_data);
          ...
          ary_data=[dictionary1  objectForKey:@"goals"];
            array_cert=[[ary_data objectAtIndex:0]objectForKey:@"PROGRESS_DATA"];
            self.lbl_TopTitle.text =[[ary_data objectAtIndex:0]objectForKey:@"GOAL_TITLE"];;
            NSLog(@"array------%@",array_cert);
...
-(void)delete
{
    ...
    ip_url *obj123=[[ip_url alloc]init];
    NSString *str123=[obj123 ipurl];
    NSString*str_checklogin=[NSString stringWithFormat:@"/student_info.php?staff_id=%@&school_id=%@&syear=%@&student_id=%@&action_type=delete_progress&progress_id=%@",STAFF_ID_K,school_id1,year_id,self.studentID,self.prog_id];
    NSLog(@"kkkkkkkkkkk%@",str_checklogin);
    NSString *url12=[NSString stringWithFormat:@"%@%@",str123,str_checklogin];
   ...
   // http://107.170.94.176/openSIS_CE6_Mobile/webservice/student_info.php?staff_id=2&school_id=1&syear=2015&student_id=13&action_type=add_progress_view
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
     ...
     //   }
       ...
       // goal_id1=self.goal_id;
        self.COURSE_PERIOD.text=[[cp_ary objectAtIndex:0]objectForKeyedSubscript:@"COURSE_PERIOD_NAME"];
        p_id=[[cp_ary objectAtIndex:0]objectForKeyedSubscript:@"COURSE_PERIOD"];
        self.P_SCALE.text=[[p_scale123 objectAtIndex:0]objectForKeyedSubscript:@"TITLE"];
        p_scale_id=[[p_scale123 objectAtIndex:0]objectForKeyedSubscript:@"VALUE"];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    if (pickerView.tag==1) {
       ...
       self.goal_id = [NSString stringWithFormat:@"%@",[[goal_ary objectAtIndex:row]objectForKey:@"GOAL_ID"]];
        FLAG_G=@"1";
...
//    [cell.delete_btn addTarget:self action:@selector(click_delete:) forControlEvents:UIControlEventTouchUpInside];
  ...
  //  NSDateFormatter *df = [[NSDateFormatter alloc] init];
   ...
   self.prog_id=[self nullChecker:[NSString stringWithFormat:@"%@",[[array_cert objectAtIndex:indexPath.row] objectForKey:@"PROGRESS_ID"]]];
    return cell;
}
...
-(void)appearonlongwithIndex:(NSIndexPath*)indexPath
{
    ...
    LBL_TITLE.text = [[array_cert objectAtIndex:currentIndex.row] objectForKey:@"PROGRESS_NAME"];
...
//    
  ...
  // self.txt_view.text  = [self nullChecker:[NSString stringWithFormat:@"%@",[[array_cert objectAtIndex:sender.tag] objectForKey:@"COMMENT"]]];
    ...
    UIStoryboard *storyBoard = [UIStoryboard storyboardWithName:@"Student" bundle:[NSBundle mainBundle]];
    NSMutableDictionary *dic_value=[array_cert objectAtIndex:currentIndex.row];
  
   progedit *mia = [storyBoard instantiateViewControllerWithIdentifier:@"progedit"];
//    
  mia.goal_name=self.goal_name123.text;
...
//    [self.navigationController pushViewController:mia1 animated:YES];
   // view_upper.hidden=YES;
    ...
    view_upper.hidden=YES;
    view_info.hidden=NO;
    self.table_cert.frame=CGRectMake(-18,150, 340,340);
