...
- (void)viewDidLoad {
    ...
    [self dodesignsforTextViews:txt_cc];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if (str.length > 0 && [self.userIsEditindToField isEqualToString:@"0"]) {
        ...
        txt_cc.text = [str2 substringToIndex:str2.length-1];
    }
    ...
    {
        if ([txt_cc.text isEqualToString:@""]) {
            txt_cc.text = self.str_cc;
        }
        else if (self.str_cc.length < 1) {
            NSLog(@"dhuklo2");
            txt_cc.text = txt_cc.text;
        }
        else  {
            txt_cc.text = [txt_cc.text stringByAppendingString:[NSString stringWithFormat:@", %@", self.str_cc]];
        }
    }
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if (isBackSpace != -8 ) {
        ...
        else if (textView == txt_cc && ![text isEqualToString:@"\n"])
        {
...
- (void)savedataongroups
{
    ...
    //
     NSString *txtToCCUser = [txt_cc.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
