...
@interface S_TrashViewController ()
{
...
@implementation S_TrashViewController
...
{
    UIStoryboard *sb=[UIStoryboard storyboardWithName:@"StudentDashboard" bundle:nil];
    Msg_compose_ViewController *msgcm=[[Msg_compose_ViewController alloc]init];
    msgcm=[sb instantiateViewControllerWithIdentifier:@"msgcompose"];
...
- (IBAction)action_DeleteMembers:(id)sender {
    ...
    for (int i = 0; i < cellindex.count; ++i) {
        ...
        NSString *STR=[NSString  stringWithFormat:@"%@_%@",[[self.ary_data1 objectAtIndex:k1] objectForKey:@"MAIL_ID"],[[self.ary_data1 objectAtIndex:k1] objectForKey:@"STATUS"]] ;
        member_ids = [member_ids stringByAppendingString:STR];
...
-(void)trashdata
{
    ...
    s_year=[NSString stringWithFormat:@"%@",[appDelegate.dic objectForKey:@"SYEAR"]];
        ip_url *obj123=[[ip_url alloc]init];
    NSString *str123=[obj123 ipurl];
    NSString*str_checklogin=[NSString stringWithFormat:@"/student/stu_trash.php?student_id=%@&school_id=%@&syear=%@&view=trash&mail_ids=%@",student_id,s_year_id,s_year,member_ids];
    NSLog(@"kkkkkkkkkkk%@",str_checklogin);
    NSString *url12=[NSString stringWithFormat:@"%@%@",str123,str_checklogin];
...
-(void)loaddata1
{
    ...
    ip_url *obj123=[[ip_url alloc]init];
    NSString *str123=[obj123 ipurl];
    NSString*str_checklogin=[NSString stringWithFormat:@"/student/stu_message_inbox.php?student_id=%@&school_id=%@&syear=%@&view=%@",student_id,s_year_id,s_year,view_select];
    NSLog(@"kkkkkkkkkkk%@",str_checklogin);
    NSString *url12=[NSString stringWithFormat:@"%@%@",str123,str_checklogin];
...
- (void)viewDidLoad {
    ...
    course_period_title=[[NSMutableArray alloc]initWithObjects:@"Inbox",@"Outbox",@"Groups",@"Compose", nil];
...
-(void)pickerDoneClicked1
{
    ...
    if ([term_flag isEqualToString:@"1"]) {
        ...
        else  if ([inbox_data isEqualToString:@"Compose"]) {
            UIStoryboard *sb=[UIStoryboard storyboardWithName:@"StudentDashboard" bundle:nil];
            Msg_compose_ViewController *msgcm=[[Msg_compose_ViewController alloc]init];
            msgcm=[sb instantiateViewControllerWithIdentifier:@"msgcompose"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"----------");
    UIStoryboard *s1=[UIStoryboard storyboardWithName:@"StudentDashboard" bundle:nil];
    MsgvwViewController *obj=[s1 instantiateViewControllerWithIdentifier:@"msgvw"];
...
- (TableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    status=[NSString stringWithFormat:@"%@",[[self.ary_data1 objectAtIndex:indexPath.row]objectForKey:@"STATUS"]];
